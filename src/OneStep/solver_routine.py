...
from user_interface import config_training_window, save_graph_dialog, save_output_dialog, save_weights_dialog
...
def one_step_solver(
    ...
    params = {"n1": n1, "n3": n3, "nte": nte, "lamb": lamb, "loss_threshold": loss_accuracy}
    ...
    save_output_btn = tk.Button(
        training_window, text="Save output", background="lightgray", command=lambda: save_output_dialog(training_window, outputs)
    )
    save_output_btn.grid(row=1, column=2, columnspan=1, padx=5, pady=5)

    save_graph_btn = tk.Button(
        training_window, text="Save graph", background="lightgray", command=lambda: save_graph_dialog(training_window, fig)
    )
