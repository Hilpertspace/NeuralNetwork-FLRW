...
@tf.function
def sliding_slice_losses(  # noqa: D103, PLR0913
    ...
    losses = loss_fn(slices)

    return tf.reshape(losses, (batch_size, number_of_slices))

@tf.function
def spatial_edge_losses(prediction: tf.Tensor) -> tf.Tensor:
...
@tf.function
def strut_losses(prediction: tf.Tensor) -> tf.Tensor:
