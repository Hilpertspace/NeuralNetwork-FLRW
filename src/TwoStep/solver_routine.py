...
from TwoStep.neural_network import Network
...
def two_step_solver(
    ...
    # Terminate two_step_solver if the training has been aborted
    if lossmin is None:
...
def run_FLRW_Net(root, n1, n2, n3, nte, lamb, epochs, lr, clipnorm, loaded_weights, l_ini, l_fin, loss_accuracy, steps):
    ...
    if loaded_weights is not None:
        two_step_solver(root, inputs=x, weights=loaded_weights, **params)
    else:
        default_weights = [[[0], [1], [0]], [[0], [0], [1], [0], [0]], [[0], [1], [0]]]
        two_step_solver(root, inputs=x, weights=default_weights, **params)
