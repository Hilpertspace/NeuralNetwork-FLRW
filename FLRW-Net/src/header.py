...
def compute_spatial_edges(sf, n3):
    ...
    edge_length = sf * np.power(12 * np.sqrt(2) * np.power(np.pi, 2.0) / n3, 1.0 / 3)
    return edge_length
...
def handle_warning_messages(
    triangulation_warning, boundary_warning, triangulation, compute_button, entry1, entry4, entry5, entry6, n1, n3, nte, ini, fin
):
    ...
    if triangulation_warning and boundary_warning:
        messagebox.showwarning("Warning", "Please choose a triangulation and the type of boundary data that you prescribe!")
    elif triangulation_warning:
        messagebox.showwarning("Warning", "Please choose a triangulation!")
    elif boundary_warning:
        messagebox.showwarning("Warning", "Please choose the type of boundary data that you prescribe!")
    else:
        ...
        if compute_value(n1, n3, lamb, ini, fin) >= compute_lambda_limit(nte):
            messagebox.showwarning(
                "Warning",
                "There is no solution to these boundary data"
