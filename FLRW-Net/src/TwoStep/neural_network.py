...
from TwoStep.layer import HiddenLayer
...
class Network(tf.keras.Model):
    ...
    def get_min_weights(self):
        ...
        return self.min_weights_2, self.min_weights_3, self.min_weights_4
    ...
    def custom_train_step(self, inputs):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            loss_strut_1 = self.EOM_struts(prediction[:,:3])
            loss_strut_2 = self.EOM_struts(prediction[:,2:])
            loss_edge = self.EOM_edges(prediction)

            loss = (loss_strut_1 + loss_strut_2 + loss_edge) / tf.constant(3,dtype=tf.float64)
