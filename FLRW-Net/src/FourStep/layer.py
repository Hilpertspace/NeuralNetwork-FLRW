...
class HiddenLayer(tf.keras.layers.Layer):
    ...
    def __init__(self):
        ...
        weights_7 = tf.constant([[0], [0], [1], [0], [0]], dtype=tf.float64)
        weights_8 = tf.constant([[0], [1], [0]], dtype=tf.float64)
        weights_9 = tf.constant([[1]], dtype=tf.float64)

        # ... and the biases
        biases = tf.constant([0, 0, 0, 0, 0, 0, 0, 0, 0], dtype=tf.float64)
        ...
        self.weights_7 = tf.Variable(initial_value=weights_7, trainable=True, dtype=tf.float64)
        self.weights_8 = tf.Variable(initial_value=weights_8, trainable=True, dtype=tf.float64)
        self.weights_9 = tf.Variable(initial_value=weights_9, trainable=False, dtype=tf.float64)
    ...
    def set_custom_weights(self, weights):
        ...
        weights_7 = tf.cast(weights[5], dtype=tf.float64)
        weights_8 = tf.cast(weights[6], dtype=tf.float64)
        ...
        self.weights_7.assign(weights_7)
        self.weights_8.assign(weights_8)
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        scaled_a4 = tf.identity(output_8)
        ...
        tmp_strut4 = self.strut_activation(output_7, output_8, output_9)

        tmp_edge2 = tf.identity(output_3)
        tmp_edge3 = tf.identity(output_5)
        tmp_edge4 = tf.identity(output_7)
        ...
        output_5 = self.spatial_edge_activation(tmp_edge2, tmp_strut2, tmp_edge3, tmp_strut3, tmp_edge4)
        output_7 = self.spatial_edge_activation(tmp_edge3, tmp_strut3, tmp_edge4, tmp_strut4, output_9)
        ...
        output_8 = self.strut_activation(output_7, scaled_a4, output_9)

        # Shape the output correctly
        output = tf.concat([output_1, output_2, output_3, output_4, output_5, output_6, output_7, output_8, output_9], axis=1)
